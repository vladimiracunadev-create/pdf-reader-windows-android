...
## Android
La selección usa el selector de archivos del sistema. El APK v0.3.1 no declara acceso amplio al almacenamiento ni `INTERNET`; solo conserva el permiso interno `DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION` generado por Android. CI abre los metadatos del APK ya compilado y falla si reaparece un permiso prohibido. `@capacitor/share` delega en la hoja del sistema sin añadir permiso de red a PDF Reader.
